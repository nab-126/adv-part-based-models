...
class SegGuidedAttack(PGDAttack):
    def __init__(
        ...
        **kwargs,
        ...
        self.loss_fn = loss_lib.CombinedLoss(
            seg_const=attack_config["seg_const"], reduction="none"
        )
        self.seg_loss_fn = loss_lib.CombinedLoss(seg_const=1, reduction="none")
        # Classification loss is used only for saving final best attack
        self.clf_loss_fn = loss_lib.CombinedLoss(seg_const=0, reduction="none")
