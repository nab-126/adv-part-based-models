...
from MaskDINO.maskdino.utils.misc import (
...
def calculate_uncertainty(logits):
    ...
    Args:
        logits (Tensor): A tensor of shape (R, 1, ...) for class-specific or
            class-agnostic, where R is the total number of predicted masks in
            all images and C is the number of foreground classes. The values are
            logits.

    Returns:
        scores (Tensor): A tensor of shape (R, 1, ...) that contains uncertainty
            scores with the most uncertain locations having the highest
            uncertainty score.
    """
...
class CustomSetCriterion(nn.Module):
    ...
    def __init__(
        ...
        self._num_clf_classes = num_clf_classes

    def loss_clf(self, outputs, targets, indices, num_masks):
        """Classification loss (NLL).

        targets dicts must contain the key "labels" containing a tensor of dim
        [nb_target_boxes]
        """
        assert "pred_clf" in outputs
        _ = num_masks  # Unused
        src_logits = outputs["pred_clf"].float()

        # TODO(design): idx is picked equal to num instances in each image.
        # Not sure if we want to do that for obj_class or keep it to 1.
        idx = self._get_src_permutation_idx(indices)
        # target_classes_o = torch.cat([t["obj_class"] for t in targets])
        target_classes_o = torch.cat(
            [t["obj_class"][J] for t, (_, J) in zip(targets, indices)]
        )
        target_classes = torch.full(
            src_logits.shape[:2],
            self._num_clf_classes,
            dtype=torch.long,
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the bounding boxes (L1 and GIoU).

        targets dicts must contain the key "boxes" containing a tensor of dim
        [nb_target_boxes, 4]. The target boxes are expected in format
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        """Compute the losses related to the masks (focal loss and dice loss).
    ...
    def _prep_for_dn(self, mask_dict):
    ...
    def _get_loss(self, loss, outputs, targets, indices, num_masks):
        loss_map = {
            "labels": self.loss_labels_ce
            if self.semantic_ce_loss
            else self.loss_labels,
            "masks": self.loss_masks,
            "boxes": self.loss_boxes_panoptic
            if self.panoptic_on
            else self.loss_boxes,
            "clf": self.loss_clf,
        }
    ...
    def forward(self, outputs, targets, mask_dict=None):
            ...
            for target in targets:
                if len(target["labels"]) > 0:
                    ...
                    output_idx = torch.tensor(
                        range(scalar), dtype=torch.long, device=device
                    )
                    output_idx *= single_pad
                    output_idx.unsqueeze_(1)
        ...
        for loss in self.losses:
            losses.update(
                self._get_loss(loss, outputs, targets, indices, num_masks)
            )
        ...
        if self.dn != "no" and mask_dict is not None:
            ...
            l_dict = {}
            l_dict["loss_bbox_dn"] = zero.clone()
            l_dict["loss_giou_dn"] = zero.clone()
            l_dict["loss_ce_dn"] = zero.clone()
            if self.dn == "seg":
                l_dict["loss_mask_dn"] = zero.clone()
                l_dict["loss_dice_dn"] = zero.clone()
            losses.update(l_dict)
            ...
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                if i >= start:
                    if self.dn != "no" and mask_dict is not None:
                        ...
                        l_dict = {}
                        l_dict[f"loss_bbox_dn_{i}"] = zero.clone()
                        l_dict[f"loss_giou_dn_{i}"] = zero.clone()
                        l_dict[f"loss_ce_dn_{i}"] = zero.clone()
                        if self.dn == "seg":
                            l_dict[f"loss_mask_dn_{i}"] = zero.clone()
                            l_dict[f"loss_dice_dn_{i}"] = zero.clone()
                        losses.update(l_dict)
        # interm_outputs loss
        if "interm_outputs" in outputs:
