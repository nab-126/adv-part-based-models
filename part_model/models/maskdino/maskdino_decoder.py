...
class CustomMaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if "clf" in mode:
            ...
            if clf_head_layers == 1:
                self.clf_embed = nn.Linear(hidden_dim, clf_head_dim)
            else:
                self.clf_embed = MLP(
                    hidden_dim, hidden_dim, clf_head_dim, clf_head_layers
                )
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["clf_head_layers"] = cfg.MODEL.MaskDINO.clf_head_layers
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if predictions_clf[0] is not None:
        ...
        if predictions_clf is not None:
            out["pred_clf"] = predictions_clf[-1]
        if self.two_stage:
    ...
    def _set_aux_loss(
        ...
        if out_clf is not None and out_clf[0] is not None:
