"""Custom MaskDINO decoder for part-based model.
...
from MaskDINO.maskdino.modeling.transformer_decoder.dino_decoder import (
    DeformableTransformerDecoderLayer,
    TransformerDecoder,
)
from MaskDINO.maskdino.modeling.transformer_decoder.maskdino_decoder import (
    TRANSFORMER_DECODER_REGISTRY,
)
from MaskDINO.maskdino.utils import box_ops
from MaskDINO.maskdino.utils.utils import (
    MLP,
...
class CustomMaskDINODecoder(nn.Module):
    """Custom MaskDINO decoder for part-based model."""
    ...
    def __init__(
        ...
        *,
        ...
        # New parameters
        mode: str = "default",
        clf_head_dim: int = 256,
    ):
        """Initialize the MaskDINO decoder.
        ...
        # EDIT: Separate classification head (works with fg and meta parts)
        self._num_obj_classes = clf_head_dim - 1
        if "clf" in mode:
            logger.info("Using classification head in MaskDINO decoder.")
            # self.clf_embed = nn.Linear(hidden_dim, clf_head_dim)
            self.clf_embed = MLP(hidden_dim, hidden_dim, clf_head_dim, 3)
            self.obj_class_enc = nn.Embedding(self._num_obj_classes, hidden_dim)
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        """Constructs a MaskDINO decoder from a config."""
        ret = {}
        ...
        # ['no', 'bitmask', 'mask2box']
        ret["initialize_box_type"] = cfg.MODEL.MaskDINO.INITIALIZE_BOX_TYPE
        ...
        ] = cfg.MODEL.SEM_SEG_HEAD.TOTAL_NUM_FEATURE_LEVELS
        ...
        # Custom parameters
        ret["mode"] = cfg.MODEL.MaskDINO.decoder_mode
        ret["clf_head_dim"] = cfg.MODEL.MaskDINO.clf_head_dim

        return ret

    def prepare_for_dn(self, targets, tgt, refpoint_emb, batch_size):
        """Prepare for the dynamic noise (dn).
        ...
        Args:
            targets: Input samples containing labels.
            tgt: Original tgt (content) in the matching part.
            refpoint_emb: Positional anchor queries in the matching part.
            batch_size: Batch size.
        """
        input_query_label, input_query_bbox, input_query_clf = None, None, None
        ...
        if not self.training:
            if refpoint_emb is not None:
                ...
                # EDIT
                input_query_clf = tgt.repeat(batch_size, 1, 1)
            else:
                input_query_label = None
                input_query_bbox = None
                input_query_clf = None
            return input_query_label, input_query_bbox, input_query_clf, attn_mask, mask_dict
        ...
        # can be modified to selectively denosie some label or boxes; also
        # known label prediction
        unmask_bbox = unmask_label = torch.cat(known)
        labels = torch.cat([t["labels"] for t in targets])
        boxes = torch.cat([t["boxes"] for t in targets])
        clf = torch.cat([t["obj_class"] for t in targets])
        batch_idx = torch.cat(
            [
        ...
        known_clf = clf.repeat(scalar, 1).view(-1)
        known_bid = batch_idx.repeat(scalar, 1).view(-1)
        known_bboxs = boxes.repeat(scalar, 1)
        known_labels_expaned = known_labels.clone()
        known_bbox_expand = known_bboxs.clone()
        known_clf_expaned = known_clf.clone()
        ...
        if noise_scale > 0:
            ...
            # EDIT: randomly put a new one here
            new_label = torch.randint_like(chosen_indice, 0, self._num_obj_classes)
            known_clf_expaned.scatter_(0, chosen_indice, new_label)
            ...
            known_bbox_expand += (
                torch.mul(
                    (torch.rand_like(known_bbox_expand) * 2 - 1.0), diff
                ).cuda()
                * noise_scale
            )
        ...
        # EDIT
        known_clf_expaned = known_clf_expaned.long().to("cuda")
        input_clf_embed = self.obj_class_enc(known_clf_expaned)
        ...
        padding_clf = torch.zeros_like(padding_label)

        if refpoint_emb is not None:
            input_query_label = torch.cat([padding_label, tgt], dim=0).repeat(
                batch_size, 1, 1
            )
            input_query_clf = torch.cat([padding_clf, tgt], dim=0).repeat(
                batch_size, 1, 1
            )
            input_query_bbox = torch.cat(
                [padding_bbox, refpoint_emb], dim=0
            ).repeat(batch_size, 1, 1)
        else:
            input_query_label = padding_label.repeat(batch_size, 1, 1)
            input_query_bbox = padding_bbox.repeat(batch_size, 1, 1)
            input_query_clf = padding_clf.repeat(batch_size, 1, 1)
        ...
        if len(known_bid):
            idx = (known_bid.long(), map_known_indice)
            input_query_label[idx] = input_label_embed
            input_query_bbox[idx] = input_bbox_embed
            input_query_clf[idx] = input_clf_embed
    ...
    def dn_post_process(
        self, outputs_class, outputs_coord, mask_dict, outputs_mask, output_clf
    ):
        ...
        output_known_clf = output_clf[:, :, : mask_dict["pad_size"], :]
        output_clf = output_clf[:, :, mask_dict["pad_size"] :, :]
        ...
        out = {
            ...
            "pred_clf": output_known_clf[-1],
        }

        out["aux_outputs"] = self._set_aux_loss(
            output_known_class,
            output_known_mask,
            out_boxes=output_known_coord,
            out_clf=output_known_clf,
        )
        mask_dict["output_known_lbs_bboxes"] = out
        return outputs_class, outputs_coord, outputs_mask, output_clf
    ...
    def _pred_box(self, reference, hs, ref0=None):
        """Predict the box from the reference and the content.
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        Args:
            ...
            mask_features: is the per-pixel embeddings with resolution 1/4 of
                the original image, obtained by fusing backbone encoder encoded
                features. This is used to produce binary masks.
        ...
        if self.two_stage:
            ...
            enc_outputs_coord_unselected = (
                self._bbox_embed(output_memory) + output_proposals
            ...
            outputs_class, outputs_mask = self._forward_prediction_heads(
            ...
            if self.initialize_box_type != "no":
                ...
                if (
                    ...
                    refpoint_embed = (
                        BitMasks(flaten_mask > 0)
                        .get_bounding_boxes()
                        .tensor.cuda()
                    )
                elif (
                    self.initialize_box_type == "mask2box"
                ):  # faster conversion
                    refpoint_embed = box_ops.masks_to_boxes(
                        flaten_mask > 0
                    ).cuda()
                else:
                    assert NotImplementedError
                refpoint_embed = (
                    box_ops.box_xyxy_to_cxcywh(refpoint_embed)
                    / torch.as_tensor([w, h, w, h], dtype=torch.float).cuda()
                )
        ...
        if self.initial_pred:
            out_boxes = self._pred_box(references, hs, refpoint_embed.sigmoid())
            assert len(predictions_class) == self.num_layers + 1
        else:
            out_boxes = self._pred_box(references, hs)
        if mask_dict is not None:
            predictions_class = torch.stack(predictions_class)
            predictions_mask = torch.stack(predictions_mask)
            predictions_clf = torch.stack(predictions_clf)
            (
                predictions_class,
                out_boxes,
                predictions_mask,
            ...
            predictions_clf = list(predictions_clf)
        elif self.training:
            # this is to insure self.label_enc participate in the model
    ...
    def _forward_prediction_heads(self, output, mask_features, pred_mask=True):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        outputs_class = self.class_embed(decoder_output)
        output_clf = self.clf_embed(decoder_output)
        ...
        return outputs_class, outputs_mask, output_clf

    @torch.jit.unused
    def _set_aux_loss(
        self, outputs_class, outputs_seg_masks, out_boxes=None, out_clf=None
    ):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        names = ["pred_logits", "pred_masks"]
        outputs = [outputs_class[:-1], outputs_seg_masks[:-1]]
        if out_boxes is not None:
            names.append("pred_boxes")
            outputs.append(out_boxes[:-1])
        if out_clf is not None:
            names.append("pred_clf")
            outputs.append(out_clf[:-1])
        return [dict(zip(names, out)) for out in zip(*outputs)]
