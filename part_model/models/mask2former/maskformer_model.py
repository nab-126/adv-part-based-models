...
from part_model.models.mask2former.criterion import SetCriterion
from part_model.models.mask2former.matcher import HungarianMatcher
...
class CustomMaskFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize CustomMaskFormer.

        Args:
            backbone: a backbone module, must follow detectron2's backbone
                interface.
            sem_seg_head: a module that predicts semantic segmentation from
                backbone features.
            criterion: a module that defines the loss
            num_queries: int, number of queries
            object_mask_threshold: float, threshold to filter query based on
            ...
            size_divisibility: Some backbones require the input height and width
                to be divisible by a specific integer. We can use this to
                override such requirement.
            sem_seg_postprocess_before_inference: whether to resize the
                prediction back to original input size before semantic
                segmentation inference or after. For high-resolution dataset
                like Mapillary, resizing predictions before inference will cause
                OOM error.
    ...
    def from_config(cls, cfg):
        """Constructs a CustomMaskFormer from a config."""
        # pylint: disable=import-outside-toplevel,unused-import
        import part_model.models.mask2former.mask_former_head

        backbone = build_backbone(cfg)
        sem_seg_head = build_sem_seg_head(cfg, backbone.output_shape())
        mask_former_cfg = cfg.MODEL.MASK_FORMER

        # Loss parameters:
        deep_supervision = mask_former_cfg.DEEP_SUPERVISION
        no_object_weight = mask_former_cfg.NO_OBJECT_WEIGHT

        # loss weights
        class_weight = mask_former_cfg.CLASS_WEIGHT
        dice_weight = mask_former_cfg.DICE_WEIGHT
        mask_weight = mask_former_cfg.MASK_WEIGHT
        ...
        matcher = HungarianMatcher(
            ...
            num_points=mask_former_cfg.TRAIN_NUM_POINTS,
        )
        ...
        if deep_supervision:
            dec_layers = mask_former_cfg.DEC_LAYERS
            aux_weight_dict = {}
        ...
        criterion = SetCriterion(
            ...
            losses=losses,
            num_points=mask_former_cfg.TRAIN_NUM_POINTS,
            oversample_ratio=mask_former_cfg.OVERSAMPLE_RATIO,
            importance_sample_ratio=mask_former_cfg.IMPORTANCE_SAMPLE_RATIO,
        )
        ...
        return {
            ...
            "criterion": criterion,
            "num_queries": mask_former_cfg.NUM_OBJECT_QUERIES,
            "object_mask_threshold": mask_former_cfg.TEST.OBJECT_MASK_THRESHOLD,
            "overlap_threshold": mask_former_cfg.TEST.OVERLAP_THRESHOLD,
            "metadata": MetadataCatalog.get(cfg.DATASETS.TRAIN[0]),
            "size_divisibility": mask_former_cfg.SIZE_DIVISIBILITY,
            "sem_seg_postprocess_before_inference": (
                mask_former_cfg.TEST.SEM_SEG_POSTPROCESSING_BEFORE_INFERENCE
                or mask_former_cfg.TEST.PANOPTIC_ON
                or mask_former_cfg.TEST.INSTANCE_ON
            ),
            "pixel_mean": cfg.MODEL.PIXEL_MEAN,
            "pixel_std": cfg.MODEL.PIXEL_STD,
            # inference
            "semantic_on": mask_former_cfg.TEST.SEMANTIC_ON,
            "instance_on": mask_former_cfg.TEST.INSTANCE_ON,
            "panoptic_on": mask_former_cfg.TEST.PANOPTIC_ON,
    ...
    def forward(
        ...
        Returns:
            List of dict each of which has the results for one image. The dict
            contains the following keys:
                * "sem_seg": A Tensor that represents the per-pixel segmentation
                    prediced by the head. The prediction has shape KxHxW that
                    represents the logits of each class for each pixel.
                * "panoptic_seg": A tuple that represent panoptic output
                    ...
                    segments_info (list[dict]): Describe each segment in
                        `panoptic_seg`. Each dict contains keys "id",
                        "category_id", "isthing".
        """
