...
class PacoMeta(torchvision.datasets.CocoDetection):
    ...
    def __init__(self, root=None, ann_path=None) -> None:
        """Initialize PartImageNet metadata class."""
        super(PacoMeta, self).__init__(root, ann_path)
...
# class PacoDataset(torchvision.datasets.CocoDetection):
class PacoDataset(PacoMeta):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, ...]:
        ...
        try:
            img, bbox_target = super(PacoDataset, self).__getitem__(index)
        except:
